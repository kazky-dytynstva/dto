...
void main() {
  group('$AudioContentDto', () {
    ...
    test('given file size is negative '
        'when creating $AudioContentDto '
        'then an AssertionError with a specific message is thrown', () {
      // Given, When, Then
      expect(
        () => AudioContentDto(fileSize: -1, duration: Duration(seconds: 100)),
        throwsAssertErrorWithMessage('File size should be positive'),
      );
    });

    test('given file size is zero '
        'when creating $AudioContentDto '
        'then an AssertionError with a specific message is thrown', () {
      // Given, When, Then
      expect(
        () => AudioContentDto(fileSize: 0, duration: Duration(seconds: 100)),
        throwsAssertErrorWithMessage('File size should be positive'),
      );
    });

    test('given duration is negative '
        'when creating $AudioContentDto '
        'then an AssertionError with a specific message is thrown', () {
      // Given, When, Then
      expect(
    ...
    test('given duration is zero '
        'when creating $AudioContentDto '
        'then an AssertionError with a specific message is thrown', () {
      // Given, When, Then
      expect(
        () => AudioContentDto(fileSize: 100, duration: Duration.zero),
        throwsAssertErrorWithMessage('Duration should be positive'),
      );
    });
